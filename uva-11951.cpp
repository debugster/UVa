...
pair<ll, ll> findMaxSum()
{
    pair<ll, ll> ans;
    ans.first = -1; // area
    ans.second = (ll) budget; // cost
    ll tempArea, tempCost;

    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            for (int k = i; k < row; k++) {
                for (int l = j; l < col; l++) {
                    tempArea = (k - i + 1) * (l - j + 1);
                    tempCost = findSum(i, j, k, l);

                    //cout << "tempArea: " << tempArea << "\ntempCost: " << tempCost << "\n\n";

                    if ((tempArea > ans.first && tempCost <= budget) || (tempArea == ans.first && tempCost < ans.second)) {
                        ans.first = tempArea;
                        ans.second = tempCost;
                    }
                }
