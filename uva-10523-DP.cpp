...
int main()
{
    ...
    for (i = 2; i <= 150; i++) {
        ...
        for (j = 0; j <= 15; j++) {
            ...
            bigMultiplication(term[j][i - 1], jStr);
            strcpy(term[j][i], PRODUCT);

            bigMultiplication(term[j][i], iStr);

            bigAddition(PRODUCT, DP[i - 1][j]);
